...
from src.datasets.data_utils import synchronized_dataloader_step
from src.datasets.dataset_optimizer import ConstantLengthDatasets
...
def get_run_name(trin_cfg, vlm_cfg):
    batch_size = f"bs{int(train_cfg.batch_size*train_cfg.gradient_accumulation_steps)}"
    max_training_steps = f"mts_{train_cfg.max_training_steps}"
    learning_rate = f"lr_vision_{train_cfg.lr_vision_backbone}-language_{train_cfg.lr_language_model}-mp_{train_cfg.lr_mp}"
    date = time.strftime("%m%d-%H%M%S")
    vit = f"{vlm_cfg.vit_model_name.split('/')[-1]}" + f"_{vlm_cfg.max_img_size}"
    mp = f"mp{vlm_cfg.pixel_shuffle_factor}"
    lm = f"{vlm_cfg.lm_model_name.split('/')[-1]}"
    return f"mini-vlm_{vit}_{mp}_{lm}_{batch_size}_{max_training_steps}_{learning_rate}_{date}"

def get_dataloaders(train_cfg, vlm_cfg):
    print(f"Getting dataloaders from {train_cfg.train_dataset_path}")
    image_processor = get_image_processor(vlm_cfg.vit_image_size, vlm_cfg.max_img_size, vlm_cfg.resize_to_max_side_len)
    tokenizer = get_tokenizer(vlm_cfg.lm_tokenizer_name, vlm_cfg.vlm_extra_tokens, vlm_cfg.lm_chat_template)

    dataset_names_to_load = train_cfg.train_dataset_names
    # Handle special loading mode
    # Shard mode: Load sharded data
    if "shards" in dataset_names_to_load:
        print("Loading shards")
        total_shards = 56
        dataset_names_to_load = [train_cfg.train_dataset_path + f"shards_{i}" for i in range(total_shards)]
    # All mode: Load all datasets
    if "all" in dataset_names_to_load:
        print("Loading all")
        dataset_names_to_load = get_dataset_config_names(train_cfg.train_dataset_path)
    
    # Load datasets and concatenates. Manually splits validation set
    combined_train_data = []
    for dataset_name in dataset_names_to_load:
        ...
        if "shard_" in dataset_name:
            try:
                ...
                print(f"Failed to load dataset shard '{dataset_name}' from '{train_cfg.train_dataset_path}'. Error: {e}")
                continue
        try:
            # streaming determines if iterable dataset fetches data from source in on demand
    ...
    if not combined_train_data:
        raise ValueError("No datasets loaded successfully. Please check the dataset paths and names.")
    ...
    # Split dataset into training and validation sets manually
    ...
    train_dataset = VQADataset(
        ...
        tokenizer,
        image_processor,
        vlm_cfg.image_token_length,
        assistant_prefix=vlm_cfg.assistant_prefix,
        assistant_suffix=vlm_cfg.assistant_suffix,
    ...
    train_dataset = ConstantLengthDatasets(train_dataset, infinite=False, max_sample_length=train_cfg.max_sample_length, seq_length=vlm_cfg.lm_max_length, num_of_sequences=train_cfg.batch_size*4, queue_size=8,
                                            max_images_per_example=train_cfg.max_images_per_example, max_images_per_knapsack=train_cfg.max_images_per_knapsack)

    val_dataset = ConstantLengthDatasets(val_dataset, infinite=False, max_sample_length=train_cfg.max_sample_length, seq_length=vlm_cfg.lm_max_length, num_of_sequences=train_cfg.batch_size*4, queue_size=8,
                                            max_images_per_example=train_cfg.max_images_per_example, max_images_per_knapsack=train_cfg.max_images_per_knapsack)
    ...
    print("Done warming up dataloader.")
...
def train(train_cfg, vlm_cfg):
    ...
    print(f"mini-vlm initialized with {sum(p.numel() for p in model.parameters()):,} parameters")
    print(f"Training summary: batch size {int(train_cfg.batch_size*train_cfg.gradient_accumulation_steps)}")
    print(f"Validation summary: batch size {int(train_cfg.batch_size*train_cfg.gradient_accumulation_steps)}")
    ...
    all_params = [p for group in optimizer.param_groups for p in groups['params']]
    ...
    while global_step < train_cfg.max_training_steps:
        ...
        for step, batch in enumerate(synchronized_dataloader_step(iter_train_loader, False)):
            is_update_step = (step + 1) % train_cfg.gradient_accumulation_steps == 0
        ...
        if is_update_step:
            ...
            # updates model parameter using gradients
            optimizer.step()
            optimizer.zero_grad()
        ...
        if train_cfg.eval_in_epochs and global_step % train_cfg.eval_interval == 0 and is_update_step:
            ...
            with torch.no_grad():
                ...
                # Save checkpoint for the evaluation step
        ...
        # Log training stats every N step (ALL RANKS must participate in collective ops)
        if global_step % train_cfg.stats_log_interval == 0 and len(accumulated_stats['tokens_per_second']) > 0 and is_update_step:
            ...
            if train_cfg.log_wandb:
                ...
                # Check for and log new llms eval results
    ...
    batch_size = int(train_cfg.batch_size*train_cfg.gradient_accumulation_steps)
