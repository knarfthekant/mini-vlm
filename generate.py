...
def main():
    ...
    tokenizer = get_tokenizer(model.cfg.lm_tokenizer_name, model.cfg.vlm_extra_tokens, model.cfg.lm_chat_template)
    ...
    image_processor = get_image_processor(model.cfg.vit_image_size, model.cfg.max_img_size, resize_to_max_side_len)
    ...
    image_string = get_image_string(tokenizer, [splitted_image_ratio], model.cfg.image_token_length)

    messages = [{"role": "user", "content": image_string + args.prompt}]
    encoded_prompt = tokenizer.apply_chat_template([messages], tokenize=True, add_generation_prompt=True, return_tensors="pt").to(device)
    ...
    for i in range(args.generations):
        gen = model.generate(input_ids, img_t, attention_mask=attention_mask, max_new_tokens=args.max_new_tokens)
